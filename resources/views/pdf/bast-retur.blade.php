...
<head>
    ...
    <title>{{$data->nomor_bast}} (Nomor BAST)</title>
    <style>
        .title{
            /* margin-top: -10px; */
        }
        ...
        <div class="main">
            ...
            <div class="title" >
                <div style="text-align: center;">
                    ...
                    <span style="font-weight:bold;">{{$data->nomor_bast}} / {{$data->timestamp}}</span>
            ...
            <div class="middle">
                <div>
                    <table style="width: 100%">
                        <tr>
                            <td>Pada Hari ini Tanggal {{date('Y-m-d',strtotime($bast->created_at))}} Pukul {{date('H:i:s',strtotime($bast->created_at))}}, telah dilakukan serah terima barang retur yang tercantum pada Surat Berita Acara Serah Terima Retur ini , yang dilakukan oleh :</td>
                        </tr>
                    </table>
                    <br>
                    <table style="width: 100%">
                            ...
                            <td style="width: 50%">
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>{{$data->approvals->where('type','Barang diserahkan')->first()->user->name ?? 0}}</td>
                                    </tr>
                                    <tr>
                                        <td>
                                            Jabatan
                                        ...
                                        <td>:</td>
                                        <td>{{$data->approvals->where('type','Barang diserahkan')->first()->user->role->name ?? 0}}</td>
                                    </tr>
                                </table>
                            </td style="width: 50%">
                            <td>
                                <table>
                                    ...
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>{{$data->approvals->where('type','Barang Diterima Pengelola Gudang')->first()->user->name ??''}}</td>
                                    </tr>
                                    <tr>
                                        <td>
                                            Jabatan
                                        ...
                                        <td>:</td>
                                        <td>{{$data->approvals->where('type','Barang Diterima Pengelola Gudang')->first()->user->role->name ?? ''}}</td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                    ...
                    <td style="width: auto;padding-right:10px;" width="auto">
                        <div style="text-align: center;display: block;">
                            <div style="margin-bottom:18px;">
                                <span style="font-size: 12px;">Dikembalikan Oleh</span>
                            </div>
                            @if ($data->approvals->where('type','Barang diserahkan')->first())
                                {!! '<img src="data:image/png;base64,' . DNS2D::getBarcodePNG(route('public-data.user',$data->approvals->where('type','Barang diserahkan')->first()->approve_by_id ?? 0), 'QRCODE',2.5,2.5) . '" alt="barcode"   />' !!}
                                <span style="display: block;font-size: 12px;">{{$data->approvals->where('type','Barang diserahkan')->first()->user->role->name ?? 0}}</span>
                                <span style="display: block;font-size: 10px;" >{{$data->approvals->where('type','Barang diserahkan')->first()->timestamp ?? '-'}}</span>
                            @else
                    ...
                    <td style="width: auto;padding-right:10px;">
                        <div style="text-align: center;">
                            <div style="margin-bottom:18px;display: block;">
                             ...
                             @if ($data->approvals->where('type','Barang Diterima Pengelola Gudang')->first())
                                {!! '<img src="data:image/png;base64,' . DNS2D::getBarcodePNG(route('public-data.user',$data->approvals->where('type','Barang Diterima Pengelola Gudang')->first()->approve_by_id ?? 0), 'QRCODE',2.5,2.5) . '" alt="barcode"   />' !!}
                                <span style="display: block;font-size: 12px;">{{$data->approvals->where('type','Barang Diterima Pengelola Gudang')->first()->user->role->name ?? 'N/A'}}</span>
                                <span style="display: block;font-size: 10px;">{{$data->approvals->where('type','Barang Diterima Pengelola Gudang')->first()->timestamp ?? '-'}}</span>
                            @else
                                <img src="{{asset('images/icon/nay.png')}}" height="72" alt="">
                                {{-- {!! '<img src="data:image/png;base64,' . DNS2D::getBarcodePNG(route('public-data.user',$data->user_id ?? 0), 'QRCODE',2.5,2.5) . '" alt="barcode"   />' !!} --}}
